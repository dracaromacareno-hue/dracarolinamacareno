...
export default function ServicesSection({ messages, locale }: { messages: ServiceMessages; locale: string }) {
  ...
  return (
    <section id="servicios" className="py-24 bg-white relative overflow-hidden">
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_rgba(201,164,97,0.04)_0%,_transparent_60%)] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <AnimatedSection className="text-center mb-16">
          ...
          <h2
            className="text-3xl sm:text-4xl lg:text-5xl font-bold text-[#211E18] leading-tight"
        ...
        <AnimatedStagger className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map(({ key, data, icon, slug }) => (
            <div
              key={key}
              className="group relative bg-white border border-[#E8E3DA] rounded-lg p-7 hover:border-[#C9A461]/40 transition-all duration-300 hover:shadow-lg hover:shadow-[#C9A461]/5 flex flex-col"
            >
              {/* Icon */}
              <div className="w-12 h-12 rounded bg-[#C9A461]/10 border border-[#C9A461]/20 flex items-center justify-center text-[#8A6B2E] mb-5 group-hover:bg-[#C9A461]/20 transition-colors">
                {icon}
              </div>
              {/* Content */}
              <h3
                className="text-[#211E18] font-semibold text-lg mb-3 group-hover:text-[#8A6B2E] transition-colors"
                style={{ fontFamily: 'var(--font-playfair-display, serif)' }}
              >
                {data.nombre}
              </h3>
              <p className="text-[#77726A] text-sm leading-relaxed mb-5 flex-1">
                {data.descripcion}
              </p>
              {/* Two actions */}
